...
- **Selectable assistant modes for the main Claude session (`/mode`)** — operators can now calibrate the main session's interaction style without forking `CLAUDE.md`. Five modes ship: `default` (no-op), `scaffolded` (restate, narrate, show the work), `socratic` (ask first, answer second), `terse-expert` (code over prose), `shipping-coach` (name the cut line, defer the rest). Modes live as small example-driven fragments under `.claude/modes/<name>.md`. Resolution precedence is `.claude/settings.json` `assistantMode` field → `.claude/mode.local` → `default`. **Variant c implementation:** a `SessionStart` hook in `.claude/settings.json` runs `scripts/resolve-mode.sh` at session start, which walks the precedence chain and emits the resolved fragment on stdout so it lands in Claude's injected context. The `/mode` slash command and the hook share the same resolver by construction (no precedence-walk duplication). Mode fragments use a uniform `Examples + Notes` structure — calibration is by demonstration, not by imperative rules. Sub-agent personas are unaffected: `/work-ticket`, `/review-pr`, etc. keep their own calibration regardless of which mode is active. Fork-extensibility: add `.claude/modes/<your-name>.md` and it survives every `/upgrade` (upstream-only overwrite rule documented in `scripts/template-sync.sh`). (#406)
