...
- The work needs sequencing or scope refinement first — that is `platform-backlog-prioritizer`.
- The work is an architectural decision (override mechanism, sync behavior, distribution classification) — that is `framework-architect`. Architecture decisions converge on one answer by design; they are not swarm-shaped.
- The ticket is a release cut — that is `release-engineer`. Releases are sequential, not fan-out.
...
2. **Definition of Ready check.** Verify all four Power Sections are present and non-empty:
   ...
   If any section is missing or empty, abort. Output a single line naming the missing section(s) and stop. Do not fabricate the missing content.

3. **Generate N distinct angles.** Read the ticket and identify the axis of variation — what is the thing the team actually wants to compare? (UX treatment? data-model shape? sync timing? fork-impact visibility?) Pick that axis, then produce N points on it that are far enough apart that a human can pick a clear winner from preview environments.
...
{1 sentence naming what these variants disagree on — UX treatment, data shape, sync timing, etc.}
...
For a hypothetical UX ticket — "Add a confirm-before-delete flow for account deletion" — a 3-variant plan might look like:
...
The same discipline applies to platform-shape tickets. For "Surface fork-impact on PR pages," distinct variants might be (a) CI-job-filled checklist in the PR description, (b) bot-posted sticky comment refreshed on push, (c) a `fork-impact` status check with a click-through detail page — three genuinely different surfaces, not three flavors of comment.
